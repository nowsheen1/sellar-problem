...
function out=sub_system_ruf_2(x1s,x2s,x3s,y1s,y2s)
rng default
global count1 y1;
FUN = @Sub_sys_1_obj;
X0 = [1;5;2];  
...
trials.X=X0;
options=optimoptions('surrogateopt','InitialPoints', trials,'MinSurrogatePoints',50,'ObjectiveLimit',10^-10,'ConstraintTolerance',10^-10','MaxFunctionEvaluations',1000);

%basically optimization stops after the max function evaluation. and
%changing max function evaluation from 300, 500,1000 does not change the
%optimal solution


[X_Sub_sys_1,fval1,exitflag1,Output_sub_system_1] = surrogateopt(@(x)Sub_sys_1_obj(x),LB,UB,options)
out=X_Sub_sys_1(1)^2+X_Sub_sys_1(2)+X_Sub_sys_1(3)-0.2*y2s;
