...
function out=sub_system_ruf(x1s,x2s,x3s,y1s,y2s)
rng default
global count1 y1;
FUN = @Sub_sys_1_obj;
X0 = [1;5;2];  
...
trials.X=X0;
options=optimoptions('surrogateopt','InitialPoints', trials,'MinSurrogatePoints',50,'ObjectiveLimit',10^-10,'MaxFunctionEvaluations',1000);
%previously constraint limit was 10^-10 but it doesnt count now. If we
%exclude the limit, answer is same.
